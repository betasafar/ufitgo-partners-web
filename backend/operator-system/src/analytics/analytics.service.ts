...
export class AnalyticsService {
  ...
  async exportReport(
    operatorId: number,
    type: "revenue" | "bookings" | "payments" | "applicant_manifest" | "financial" | "refund_logs" | "accommodation",
    format: "csv" | "pdf",
    startDate?: string,
    endDate?: string,
    options?: { visaStatus?: string; group?: string; includePhotos?: boolean },
  ) {
    const end = endDate ? new Date(endDate) : new Date()
    ...
    switch (type) {
      ...
      case "payments":
        ...
        break
      case "applicant_manifest":
        data = await this.getApplicantManifestData(operatorId, start, end, options)
        break
      case "financial":
        data = await this.getFinancialReportData(operatorId, start, end)
        break
      case "refund_logs":
        data = await this.getRefundLogsData(operatorId, start, end)
        break
      case "accommodation":
        data = await this.getAccommodationListData(operatorId, start, end)
        break
    }

    const recordCount = Array.isArray(data) ? data.length : data.transactions?.length || 0
  ...
  private async getApplicantManifestData(
    operatorId: number,
    start: Date,
    end: Date,
    options?: { visaStatus?: string; group?: string; includePhotos?: boolean },
    ...
    if (options?.visaStatus) {
      qb.andWhere("b.visaStatus = :visaStatus", { visaStatus: options.visaStatus })
    }

    if (options?.group) {
      qb.andWhere("b.group = :group", { group: options.group })
    }

    qb.orderBy("b.pilgrimName", "ASC")

    return qb.getRawMany()
  }

  private async getFinancialReportData(operatorId: number, start: Date, end: Date) {
    const revenue = await this.bookingRepo
      .createQueryBuilder("b")
      .select("COALESCE(SUM(b.amountPaid), 0)", "totalRevenue")
      .addSelect("COALESCE(SUM(b.totalAmount), 0)", "expectedRevenue")
    ...
    const transactions = await this.transactionRepo
      ...
      .select(["TO_CHAR(t.createdAt, 'YYYY-MM-DD') as date", "t.type", "t.amount", "t.status", "t.bookingId"])
  ...
  private async getRefundLogsData(operatorId: number, start: Date, end: Date) {
    return this.bookingRepo
      .createQueryBuilder("b")
      .select([
        "b.id as bookingId",
        ...
        "b.refundStatus",
        "b.refundProcessedAt",
        "b.totalAmount as originalAmount",
      ])
      .where("b.operatorId = :operatorId", { operatorId })
      .andWhere("b.refundAmount > 0")
      .andWhere("b.createdAt BETWEEN :start AND :end", { start, end })
      .orderBy("b.refundProcessedAt", "DESC")
      .getRawMany()
  }

  private async getAccommodationListData(operatorId: number, start: Date, end: Date) {
    return this.bookingRepo
      .createQueryBuilder("b")
      .select([
        "b.packageId",
      ...
      .andWhere("b.createdAt BETWEEN :start AND :end", { start, end })
      .orderBy("b.hotelName", "ASC")
      .addOrderBy("b.checkInDate", "ASC")
      .getRawMany()
  }
