...
export class WalletService {
 ...
 async getTransactionsFiltered(
  operatorId: number,
  limit: number,
  type?: "credit" | "debit" | "refund",  // ← also add "refund" if needed
  ...
  if (type) {
    // Do NOT uppercase! Keep it lowercase
  ...
  if (endDate) {
    // Better: end of day to include full day
    const end = new Date(endDate)
    end.setHours(23, 59, 59, 999)
    qb.andWhere("t.createdAt <= :endDate", { endDate: end })
  }
